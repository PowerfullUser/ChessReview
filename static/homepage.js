...
// Chess.com JSON Data
// Latest Archive
async function getMonthlyArchives(username) {
    try {
...
async function getGameData(url) {
    ...
    const pgn_data = limit_games.map((game) => game.pgn);
    const url_data = limit_games.map((game) => game.url);
    const white_username = limit_games.map((game) => game.white?.username ?? "Unknown");
    const black_username = limit_games.map((game) => game.black?.username ?? "Unknown");
    const white_rating = limit_games.map((game) => game.white?.rating ?? 0);
    const black_rating = limit_games.map((game) => game.black?.rating ?? 0);
    const list = {
        "pgn": pgn_data,
        "url": url_data,
...
function insertToList(latest_games) {
    const games_panel = document.querySelector(".games");
    if (!games_panel) {
        return;
    }
    ;
